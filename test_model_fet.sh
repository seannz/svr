...
python model_test_fet.py --trainee segment --dataset $dataset  --batch_size $batch_size --valid_batch_size $batch_size --loss $loss --aux_loss $loss --network $model --max_steps 250000 --limit_train_batches 20000 --optim adam --lr_start 1e-4 --momentum 0.90 --decay 0.0000 --schedule poly --val_check_interval 1.0 --monitor val_loss --monitor_mode min --seed $seed --remarks $remarks $mode --direct --log_every_n_steps $log_every_n_steps 
#> results/"$remarks".txt
