...
if __name__ == "__main__":
   ...
   # trainee = models.segment(model=models.flow_SNet3d1_256_4())
    print(torch.load('./checkpoints/feta3d1_4_svr_mf_flow_SNet3d1_32_l2_loss_bulktrans05_bulkrot45_trans10_rot20_250k_nobound_nosine_nomask_2stacks/best-v10.ckpt').keys())
    trainee.load_state_dict(torch.load('./checkpoints/feta3d1_4_svr_mf_flow_SNet3d1_32_l2_loss_bulktrans05_bulkrot45_trans10_rot20_250k_nobound_nosine_nomask_2stacks/best-v10.ckpt')['state_dict'])
    model = trainee.model
    model.eval()
    ...
    sets = datasets.feta3d1_4_svr_mf_clinical(subsample=1) #<-- instantiate dataset with no subsampling (=1) as we will do that ourselves
    imgnum=0 # pick any image num
    ...
    # SAVE VOLUMES
   # imageio.volwrite('finput_test_fet_lasttrain50.tif', input[0,0,0,:,:,:])
   # imageio.volwrite('fgtruth_test_lasttrain50.tif', truth[0,0,:,:,:])
   # imageio.volwrite('v10_fet_pred_clinical.tif',splat.detach().numpy()[0,0,:,:,:] )
...
# Save the NIfTI image to a file
  # SAVE AS NIFTY
    ...
    nib.save(nii_image, 'v10clin_sytnthoutput_final_model2.nii')

    nii_image = nib.Nifti1Image(truth.numpy()[0,0,:,:,:], affine=np.eye(4))  # You might need to specify the affine transformation matrix
    nib.save(nii_image, 'v10clin_synthintput_final_model2.nii')


    nii_image = nib.Nifti1Image(input.numpy()[0,0,0,:,:,:], affine=np.eye(4))  # You might need to specify the affine transformation matrix
    nib.save(nii_image, 'v3_synthgt_final_model_2.nii')
  
    # print("flow shape")
    #print(flow.shape)
    #imageio.volwrite('guess_flowx_v1.tif', flow[0,0,:,:].detach().numpy())
    #true_flow = item[1][None][:,:3,0]
    #imageio.volwrite('true_flowx.tif', true_flow[0,0,:,:].detach().numpy())
